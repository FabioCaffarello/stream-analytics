...
func NewVolumeProfileWriter(pool ...*Pool) *VolumeProfileWriter {
	w := &VolumeProfileWriter{
...
func NewPgVolumeProfileWriter(pool *Pool) *VolumeProfileWriter {
	if pool == nil {
		return NewVolumeProfileWriter()
	}
	w := NewVolumeProfileWriter()
	w.exec = pool
	return w
}

func (w *VolumeProfileWriter) UpsertVolumeProfileBucket(ctx context.Context, upsert insightsports.VolumeProfileBucketUpsert) *problem.Problem {
	if w == nil {
