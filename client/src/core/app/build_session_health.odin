...
// S59: Session Health / Diagnostics page — consolidates backend session dashboard
...
@(private = "package")
page_session_health_enter :: proc(state: ^App_State) {
	state.session_health = {}
	fetch_session_health(state)
}

@(private = "package")
page_session_health_leave :: proc(state: ^App_State) {
	state.session_health = {}
}
...
@(private = "file")
fetch_session_health :: proc(state: ^App_State) {
	if state.marketdata.fetch_session_dashboard == nil {
		state.session_health.fetch_status = .Error
		return
	}
	...
	state.session_health.view = result
	state.session_health.fetch_status = .Success
	state.session_health.fetch_frame = state.frame
}
...
@(private = "package")
page_session_health_render :: proc(state: ^App_State, workspace: ui.Rect, pointer: ui.Pointer_Input) {
	...
	// --- Header ---
	ui.push_text(&state.cmd_buf, {x, y}, "Session Health",
...
draw_health_session_status :: proc(
	state: ^App_State, view: ^services.Session_Health_Result,
...
@(private = "package")
page_session_health_render_detail :: proc(state: ^App_State, rect: ui.Rect, pointer: ui.Pointer_Input) {
	y := rect.pos.y
