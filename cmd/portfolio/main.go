// Package main is the market-raccoon portfolio binary.
//
// The portfolio service consumes `execution.event` and emits deterministic
// projected `portfolio.state` snapshots.
package main
...
func main() {
	...
	if prob != nil {
		fmt.Fprintf(os.Stderr, "portfolio: config error: %v\n", prob)
		os.Exit(1)
	}
