...
if [[ -x "${ROOT_DIR}/scripts/check-pack-subjects.sh" ]]; then
  if [[ "$mode" == "check" ]]; then
    if ! "${ROOT_DIR}/scripts/check-pack-subjects.sh"; then
      subject_guard_status=1
    fi
  else
    "${ROOT_DIR}/scripts/check-pack-subjects.sh" --fix-hints || true
  fi
else
