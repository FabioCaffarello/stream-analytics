...
func TestGoldenReplay(t *testing.T) {
	...
	goldenPath := filepath.Join("testdata", "replay", "golden", "output-mini.jsonl")
	shouldUpdate := *updateGoldenFlag

	ensureMiniInputFixture(t, inputPath, 50, shouldUpdate)
	...
	if p := replay.CompareFixtureFiles(outputPath, goldenPath); p != nil {
...
func runReplayToPath(t *testing.T, inputPath string) (string, replay.ReplaySummary) {
	t.Helper()
	fakeClock := clock.NewFakeClock(time.UnixMilli(0))
	player, p := replay.NewPlayer(inputPath, fakeClock, contracts.BootstrapPayloadCodecRegistry)
	if p != nil {
	...
	if p := replay.WriteFixtureFromEnvelopes(outputPath, capture.Envelopes()); p != nil {
...
func ensureMiniInputFixture(t *testing.T, inputPath string, n int, allowCreate bool) {
	...
	if p := replay.WriteFixtureFromEnvelopes(inputPath, envs); p != nil {
