...
func TestRecordFromSourceWritesFixture(t *testing.T) {
	...
	summary, p := RecordFromSource(context.Background(), src, outPath, 0, time.Time{})
	if p != nil {
...
func TestRecordFromSourceHonorsUntilAndMaxN(t *testing.T) {
	...
	summary, p := RecordFromSource(context.Background(), src, outPath, 1, until)
	if p != nil {
		t.Fatalf("RecordFromSource: %v", p)
	}
...
func TestRecordFromSourceCloseError(t *testing.T) {
	...
	_, p := RecordFromSource(context.Background(), src, filepath.Join(t.TempDir(), "source-error.jsonl"), 0, time.Time{})
	if p == nil {
...
func TestRecordFromSourceDeterministicOutput(t *testing.T) {
	...
	if _, p := RecordFromSource(context.Background(), srcA, outA, 0, time.Time{}); p != nil {
		t.Fatalf("RecordFromSource A: %v", p)
	}
	if _, p := RecordFromSource(context.Background(), srcB, outB, 0, time.Time{}); p != nil {
		t.Fatalf("RecordFromSource B: %v", p)
	}
	if p := CompareFixtureFiles(outA, outB); p != nil {
