...
// S59: Delivery Health parser — JSON deserialization for GET /api/v1/session/dashboard.
// Backend-owned composed read model for delivery-level health and diagnostics:
// instrument freshness, resync coverage, artifact availability, venue counts.
...
//
// Renamed from Session_Health → Delivery_Health (S159) to align ubiquitous language:
// this data reflects the backend's ability to deliver artifacts, not WS connection health.
...
Delivery_Health_Freshness :: struct {
	status:             string,
...
Delivery_Health_Artifact_Coverage :: struct {
	status:                  string,
...
Delivery_Health_Artifact :: struct {
	name:              string,
	endpoint:          string,
	default_timeframe: string,
	timeframes:        [HEALTH_TF_CAP]string,
	tf_count:          int,
	coverage:          Delivery_Health_Artifact_Coverage,
}

Delivery_Health_Summary :: struct {
	venues:      int,
	instruments: int,
}

Delivery_Health_Result :: struct {
	server_time_ms:   i64,
	status:           string,
	// Readiness.
	readiness_status: string,
	// Freshness.
	freshness:        Delivery_Health_Freshness,
	// Resync / delivery.
	resync:           Delivery_Health_Resync,
	// Artifacts coverage.
	artifacts:      [HEALTH_ARTIFACT_CAP]Delivery_Health_Artifact,
	artifact_count: int,
	// Summary.
	summary:        Delivery_Health_Summary,
}
...
delivery_health_parse_json :: proc(data: []u8, out: ^Delivery_Health_Result) -> bool {
	...
	// Freshness.
	out.freshness = Delivery_Health_Freshness{
	...
	// Resync.
	out.resync = Delivery_Health_Resync{
	...
	for art in root.artifacts {
		...
		a.coverage = Delivery_Health_Artifact_Coverage{
	...
	// Summary.
	out.summary = Delivery_Health_Summary{
