...
OUT_FILE=".context/evidence/c4-production-soak.txt"
...
echo "# C4 production soak harness $(date -u +%Y-%m-%dT%H:%M:%SZ)" | tee -a "$OUT_FILE"
echo "go_cache=$GO_CACHE_DIR" | tee -a "$OUT_FILE"

echo "Running pipeline soak (10M events, 4 exchanges)..." | tee -a "$OUT_FILE"
...
echo "Running WS delivery soak (50 clients)..." | tee -a "$OUT_FILE"
MR_ENABLE_SOAK=1 GOCACHE="$GO_CACHE_DIR" \
	go test ./internal/interfaces/ws -run 'TestSoak_WSDelivery_SlowClients' \
	-count=1 -v -timeout=20m 2>&1 | tee -a "$OUT_FILE"

echo "C4 production soak completed" | tee -a "$OUT_FILE"
