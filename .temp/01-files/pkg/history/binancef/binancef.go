...
func NewDateHandler(from time.Time, to time.Time) *DateHandler {
	return &DateHandler{
		from: from,
		to:   to,
	}
...
func (d *DateHandler) GetDates() []Date {
	var dates []Date
	current := d.from

	for !current.After(d.to) {
...
func (b *BinanceFutureZip) parseAggTrade(row []string) *event.Trade {
	...
	pair := &event.Pair{
		Exchange: "binancef",
		Symbol:   strings.ToLower(b.symbol),
	}
	return &event.Trade{
		Price: price,
		Qty:   qty,
...
func (b *BinanceFutureZip) downloadAndUnzip(url string) (io.ReadCloser, error) {
	...
	csvName := fmt.Sprintf("%s/%s-%s-%s-%s.csv", dataDir, b.symbol, b.year, b.month, b.day)

	// check if csv file exists
	if _, err := os.Stat(csvName); err == nil {
		fmt.Println("csv file already exists, skipping download")
		return os.Open(csvName)
	}

	// check if zip file exists
	zipFile, err := os.Open(zipName)
	if err == nil {
		fmt.Println("zip file exists, unzipping...")
		defer zipFile.Close()
	...
	// neither file exists, download and unzip
	resp, err := http.Get(url)
	if err != nil {
		return nil, err
	}
