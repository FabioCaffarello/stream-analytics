...
| `core/signals` | `core/evidence/domain` | Composition layer reads evidence types |
| `core/execution` | `core/strategy/domain` | Execution receives strategy intents |
| `core/portfolio` | `core/execution/domain` | Portfolio projects from execution events |
...
| `core/evidence` | `core/signals` | Evidence is upstream of signals, never downstream |
| `core/strategy` | `core/execution` | Strategy emits intents; execution consumes them. No back-channel. |
...
|--------|-----------------|----------------------|----------------|
| `cmd/consumer` | `actors/marketdata` | marketdata, marketmodel | Exchange WS, NATS publish |
| `cmd/processor` | `actors/aggregation`, `actors/insights` | aggregation, insights, marketdata | NATS subscribe, NATS publish |
| `cmd/server` | `actors/delivery` | delivery | WS server, NATS subscribe |
| `cmd/signals` | `actors/signal`, `actors/signals` | signal, signals, evidence | NATS subscribe/publish |
| `cmd/strategist` | `actors/strategy` | strategy | NATS subscribe/publish |
| `cmd/executor` | `actors/execution` | execution, strategy | NATS subscribe, exchange API |
| `cmd/portfolio` | `actors/portfolio` | portfolio, execution | NATS subscribe, TimescaleDB |
| `cmd/store` | — (no actors) | — | TimescaleDB, ClickHouse, NATS subscribe |
| `cmd/backfill` | — (no actors) | marketdata | Exchange REST, TimescaleDB |
| `cmd/migrate` | — (no actors) | — | Goose migrations |
| `cmd/credentials-broker` | — (no actors) | — | Vault/env, HTTP |
...
shared ──────────────────────────────────────────────────────
  ...
  ├─ signals ──────→ evidence/domain   (compile-time only)
  │
  ├─ strategy (isolated)
  ├─ execution ────→ strategy/domain   (compile-time only)
  ├─ portfolio ────→ execution/domain  (compile-time only)
  │
...
| **signals** | NATS events from evidence | `CompositeSignalV1` → NATS | `evidence/domain`, `shared` |
| **strategy** | NATS events from signals | `StrategyIntentV1` → NATS | `shared` |
| **execution** | NATS intents from strategy | `ExecutionEventV1` → NATS | `strategy/domain`, `shared` |
| **portfolio** | NATS events from execution | `PortfolioStateV1` → NATS/DB | `execution/domain`, `shared` |
...
|------|------------|---------------|
| N1 | Packages differing only by singular/plural | `signal/` vs `signals/` is a tracked debt (S1) |
...
```go
// BAD: core/signals/app/composer.go
import "github.com/market-raccoon/internal/core/aggregation/app"
func (c *Composer) Evaluate() {
    candles := aggapp.GetLatestCandles(...)  // Direct call across contexts
}

// GOOD: actors/signals/runtime/consumer.go
func (a *SignalConsumer) Receive(ctx actor.Context) {
    // Receives CandleV2 events via NATS subscription — no direct import
    switch msg := ctx.Message().(type) {
    case *envelope.Envelope:
        a.composer.Evaluate(msg.Payload)
    }
}
```
