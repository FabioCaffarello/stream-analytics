...
| **W5** | `cmd/backfill/` é stub funcional | Apenas bootstrap test; sem download de archives de exchange; gap no operational tooling vs MarketMonkey |
| **W6** | Gap detection sem repair | Depth gap detectado em telemetria (log only); sem mecanismo de autocorreção ou alerta estruturado |
...
| **O3** | Backfill operacional via `cmd/backfill` | Replay infra + cold readers (C3) provêm o alicerce; falta apenas o download adapter |
...
| **W5** Backfill stub | — | — | **Invest: Implementar download adapter + wiring com replay** | — | **Mitigate: Gap crítico vs MM — sem backfill, sem historical data** | — | — |
...
### 2. Backfill operacional é o gap competitivo mais crítico
**W5 + W6 + O3 + T1 → Ação:** Implementar C3 (download adapters + gap detection + cold readers). A infraestrutura de replay já existe; falta o "último quilômetro" — adapter de download e wiring no `cmd/backfill`. Sem isso, historical data é impossível.
...
| Cobertura Funcional | **3/5** | Stats ausente, heatmap não fiado, backfill stub, exchanges assimétricos |
| Prontidão Operacional | **3/5** | Config/shutdown/readiness OK; falta backfill, gap repair, migration runner |
...
**Score Geral: 3.9 / 5.0** — Fundação técnica excepcional, execução funcional em catch-up.
...
| **P0** | Implementação | C3: `cmd/backfill` operacional + gap detection com repair |
