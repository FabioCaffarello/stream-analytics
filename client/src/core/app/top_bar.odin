...
draw_venue_dropdown :: proc(state: ^App_State, pointer: ui.Pointer_Input, viewport_w: f32) {
	...
	if reg == nil || reg.count < 1 do return
	if viewport_w < 500 do return // not enough space on narrow screens

	// Build options from stream_views.
	venue_labels: [VENUE_DROPDOWN_MAX]string
	venue_ids:    [VENUE_DROPDOWN_MAX]u64
	venue_label_bufs: [VENUE_DROPDOWN_MAX][32]u8
	vc := 0
	for i in 0 ..< STREAM_VIEW_CAP {
		if !reg.slots[i].used do continue
		if vc >= VENUE_DROPDOWN_MAX do break
		slot := &reg.slots[i]
		...
		if slot.has_stream_info && len(slot.stream_info.venue) > 0 {
			venue_label_bufs[vc] = {}
			_ = fmt.bprintf(venue_label_bufs[vc][:], "%s:%s",
				slot.stream_info.venue, slot.stream_info.symbol)
			slen := 0
			for bi in 0 ..< 32 {
				if venue_label_bufs[vc][bi] == 0 do break
				slen += 1
			}
			venue_labels[vc] = string(venue_label_bufs[vc][:slen])
		} else {
			venue_labels[vc] = "---"
		}
		venue_ids[vc] = slot.subject_id
		vc += 1
	}
	if vc < 1 do return

	// Sync dropdown selected_idx to active stream.
	if reg.has_active {
		for vi in 0 ..< vc {
			if venue_ids[vi] == reg.active_subject_id {
				state.venue_dropdown.selected_idx = vi
				break
			}
		}
	}

	// Position: row 1 of topbar, right of hero text area.
	// Fixed position to avoid depending on draw_top_bar cursor state.
	dd_w := f32(130)
	dd_h := f32(18)
	dd_x := viewport_w - dd_w - 180 // left of connection badge area
	if dd_x < 200 do dd_x = 200
	dd_y := (TOP_BAR_H - dd_h) * 0.5 // centered in top bar

	trigger_rect := ui.rect_xywh(dd_x, dd_y, dd_w, dd_h)
	dd_result := ui.dropdown(&state.cmd_buf, trigger_rect, venue_labels[:vc],
		&state.venue_dropdown, pointer, state.text.measure, ui.FONT_SIZE_XS)

	if dd_result.changed && dd_result.selected_idx >= 0 && dd_result.selected_idx < vc {
		new_sid := venue_ids[dd_result.selected_idx]
		if new_sid != reg.active_subject_id {
			reg.active_subject_id = new_sid
			reg.has_active = true
			...
			show_toast(state, venue_labels[dd_result.selected_idx])
		}
	}
}
